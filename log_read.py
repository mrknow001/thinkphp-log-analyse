...
def main_task(ui):
    ...
    #dates = ['2020-11-02','2020-08-19','2021-01-13']
    for date in dates:
        #date = '21_01_13'
        if custom_url == '':
            if default_url == '':
...
def read_log(ui,url,date,rule):
    ...
    for user_pass in set(user_passs):
        #print(user_pass)
        if (user_pass[2] in user_list):
            item = ui.tableWidget.findItems(user_pass[2], Qt.MatchExactly)
            row = item[0].row()
            password = ui.tableWidget.item(row, 2).text()
            if password == user_pass[4]:
                continue
